...
def create_fixtures_table(cursor):
    cursor.execute('''DROP TABLE IF EXISTS fixturesallleagues''')
    cursor.execute('''
    CREATE TABLE fixturesallleagues (
...
try:
    ...
    # Liga ID-k és szezonok lekérdezése a megadott feltételekkel
    cursor.execute('''
    SELECT DISTINCT id, season_year 
    FROM leagues 
    WHERE type = 'League'
    ''')
    ...
    for league_id, season_year in leagues_data:
        ...
        try:
            ...
            for fixture in fixtures_data['response']:
                try:
                    cursor.execute('''
                    INSERT OR REPLACE INTO fixturesallleagues VALUES (
                        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
                    )
                    ''', (
                    ...
                    logging.error(f"Hiba a mérkőzés mentése közben: {str(e)}, Fixture ID: {fixture['fixture']['id']}")
                    continue

            conn.commit()
            logging.info(f"Sikeresen mentve a mérkőzések a {league_id} ligához, {season_year} szezonhoz")
            ...
            logging.error(f"Váratlan hiba a {league_id} liga feldolgozása közben: {str(e)}")
            continue
