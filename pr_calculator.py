...
log_filename = os.path.join(log_dir, f'pr_calculator_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
def create_pr_tables():
    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS pr_helper (
        ...
        # PR értékek tárolása
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS match_pr_data (
                ...
                FOREIGN KEY (fixture_id) REFERENCES fixturesallleagues(id)
            )
        ''')
        
        conn.commit()
        logger.info("PR táblák sikeresen létrehozva")
...
def calculate_power_rankings():
    ...
    try:
        ...
        logger.info("Power Rankings számítás kezdése")
        
        # Mérkőzések lekérése időrendi sorrendben
        cursor.execute('''
            SELECT id, league_id, league_season, home_team_id, away_team_id, 
                   goals_home, goals_away, date
            FROM fixturesallleagues
        ...
        for match in matches:
            ...
            # Mérkőzésszám növelése mindkét csapatnál az adott szezonban
            ...
            if goals_home is not None and goals_away is not None:
                ...
                # PR értékek mentése a match_pr_data táblába
                cursor.execute('''
                    INSERT OR REPLACE INTO match_pr_data 
                    (fixture_id, home_pr, away_pr, pr_diff, include_in_stats)
                    VALUES (?, ?, ?, ?, ?)
                ''', (match_id, home_pr, away_pr, pr_diff, include_in_stats))
        
        conn.commit()
        logger.info("Power Rankings számítás befejezve")
        ...
        logger.error(f"Hiba a Power Rankings számításakor: {str(e)}")
    finally:
        conn.close()
...
def calculate_pr_diff_statistics(cursor):
    ...
    try:
        logger.info("PR különbségek statisztikáinak számítása kezdődik")
        
        # PR helper tábla ürítése
        cursor.execute('DELETE FROM pr_helper')
        ...
        cursor.execute('''
            WITH match_results AS (
                ...
                FROM match_pr_data m
                JOIN fixturesallleagues f ON m.fixture_id = f.id
                WHERE f.goals_home IS NOT NULL 
                AND f.goals_away IS NOT NULL
                AND m.include_in_stats = 1
                GROUP BY ROUND(m.pr_diff, 1)
            )
            INSERT INTO pr_helper
            SELECT 
                pr_diff,
                total_matches,
                home_wins,
        ...
        logger.info("PR különbségek statisztikái sikeresen kiszámítva")
        
    except Exception as e:
        logger.error(f"Hiba a PR különbségek statisztikáinak számításakor: {str(e)}")

def update_match_probabilities(cursor):
    """Mérkőzések végeredmény valószínűségeinek frissítése"""
    try:
        logger.info("Mérkőzés valószínűségek frissítése kezdődik")
        
        cursor.execute('''
            UPDATE match_pr_data
            SET 
                home_win_prob = (
                    SELECT home_win_percentage
                    FROM pr_helper
                    WHERE ROUND(pr_helper.pr_diff, 1) = ROUND(match_pr_data.pr_diff, 1)
                ),
                draw_prob = (
        ...
        logger.info("Mérkőzés valószínűségek sikeresen frissítve")
        
    except Exception as e:
        logger.error(f"Hiba a mérkőzés valószínűségek frissítésekor: {str(e)}")

if __name__ == "__main__":
    try:
        logger.info("=" * 50)
        logger.info("PR Calculator indítása")
        logger.info(f"Futtatás időpontja: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        
        create_pr_tables()
        calculate_power_rankings()
        
        logger.info("PR Calculator futás sikeresen befejezve")
        logger.info("=" * 50)
    except Exception as e:
